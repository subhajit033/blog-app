...
const Header = () => {
  ...
  const categories:{label: string, value: string}[] = [{label: 'All', value: 'all'}, ...statuses];
  ...
  return (
    ...
    <div className='slider-container w-[36rem]  border border-gray-400 rounded-full'>
      ...
      {categories.map((cat, i) => {
          return (
            <p onClick={()=> {setActive(cat.label)}} key={i} className={`px-4 py-2 ${active === cat.label ? 'bg-gray-300': ''} rounded-full cursor-pointer`}>
              {cat.label}
            </p>
          );
        })}
